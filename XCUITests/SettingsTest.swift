...
class SettingsTest: BaseTestCase {
    func testHelpOpensSUMOInTab() {
        ...
        waitForExistence(app.webViews.staticTexts["Beacon for iOS Support"])
    ...
    /* Disable test since the option to set Beacon is not available in this build after issue #8513 landed, issue #8627
    func testDefaultBrowser() {
        // A default browser card should be available on the home screen
        if #available(iOS 14, *) {
            waitForExistence(app.staticTexts["Set links from websites, emails, and Messages to open automatically in Beacon."], timeout: 5)
