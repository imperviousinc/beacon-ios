...
/// Handshake cert verification errors
var hnsCertErrors = [String : NSError]()

let HNSErrorNotSynced = NSError.init(domain: "HNSErrorNotReady", code: Int(MobileHNSNotSynced), userInfo: [
    NSLocalizedDescriptionKey: "Client is still syncing please wait ..."
])

let HNSErrorBogus = NSError.init(domain: "HNSErrorBogus", code: Int(MobileHNSBogus), userInfo: [
    NSLocalizedDescriptionKey: "DANE verification failed."
])

let HNSErrorLookupTimeout = NSError.init(domain: "HNSErrorTimeout", code: Int(MobileHNSLookupTimeout), userInfo: [
    NSLocalizedDescriptionKey: "TLSA lookup timed out. Please try again."
])

let HNSErrorPeerTimeout = NSError.init(domain: "HNSErrorTimeout", code: Int(MobileHNSPeerTimeout), userInfo: [
    NSLocalizedDescriptionKey: "Failed requesting proof to verify this site. Please try again."
])

let HNSErrorNoPeers = NSError.init(domain: "HNSErrorNotReady", code: Int(MobileHNSNoPeers), userInfo: [
    NSLocalizedDescriptionKey: "No peers available to verify this site. This may take a few seconds ..."
])
...
extension BrowserViewController: WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: Error) {
        ...
        if error.code == Int(CFNetworkErrors.cfurlErrorCancelled.rawValue) {
            if let url = error.userInfo[NSURLErrorFailingURLErrorKey] as? URL {
                var origin = url.hostPort ?? ""
                if url.port == nil && url.scheme == "https" {
                    origin += ":443"
                }
                
                if let hnsErr = hnsCertErrors[origin] {
                    hnsCertErrors[origin] = nil
                    ErrorPageHelper(certStore: profile.certStore).loadPage(hnsErr, forUrl: url, inWebView: webView)
                    return
                }
            }
    ...
    func webView(_ webView: WKWebView, didReceive challenge: URLAuthenticationChallenge, completionHandler: @escaping (URLSession.AuthChallengeDisposition, URLCredential?) -> Void) {
        ...
        // handle certificate challenge
        let origin = "\(challenge.protectionSpace.host):\(challenge.protectionSpace.port)"
        if challenge.protectionSpace.authenticationMethod == NSURLAuthenticationMethodServerTrust,
           let trust = challenge.protectionSpace.serverTrust {
            guard let cert = SecTrustGetCertificateAtIndex(trust, 0) else {
                // hard fail if we can't read the certificate
                // for some reason
                completionHandler(.cancelAuthenticationChallenge, nil)
                return
            }
            
            // See if the certificate has previously been
            ...
            // TLDs excluded from DNS-Based Authentication
            if !CanAuthWithDANE(hostname: challenge.protectionSpace.host) {
            ...
            // clear existing errors for this origin
            hnsCertErrors[origin] = nil
            
            // Verify DANE in a background thread
            DispatchQueue.global(qos: .userInteractive).async {
                
                let verifyResult = HandshakeCtx?.verifyCerts(certData, port: String(challenge.protectionSpace.port), proto:"tcp",hostname: challenge.protectionSpace.host)
                
                let result = verifyResult?.code() ?? Int(MobileHNSBogus)
                
                DispatchQueue.main.async {
                    if result == MobileHNSSecure {
                        print("dane: host [\(challenge.protectionSpace.host)]: accepting")
                        completionHandler(.useCredential, URLCredential(trust: trust))
                        return
                    }
                    if result == MobileHNSInsecure {
                        print("dane: host [\(challenge.protectionSpace.host)]: default handling")
                        completionHandler(.performDefaultHandling, nil)
                        return
                    }
                    
                    if result == MobileHNSNotSynced {
                        let height = HandshakeCtx?.height() ?? -1
                        let progress = Int((HandshakeCtx?.progress() ?? 0) * 100)
                        let errMessage = "Client is still syncing (height: \(height), progress: \(progress)). Please wait."
                        
                        hnsCertErrors[origin]  = NSError.init(domain: "HNSErrorNotReady", code: Int(result), userInfo: [
                            NSLocalizedDescriptionKey: errMessage
                        ])
                        
                    } else if result == MobileHNSLookupTimeout {
                        hnsCertErrors[origin] = HNSErrorLookupTimeout
                    } else if result == MobileHNSPeerTimeout {
                        hnsCertErrors[origin] = HNSErrorPeerTimeout
                    } else if result == MobileHNSNoPeers {
                        let poolSize = HandshakeCtx?.peerCount() ?? -1
                        let activePeers = HandshakeCtx?.activePeerCount() ?? -1
                        let errMessage = "No peers available to verify this site (pool: \(poolSize), active: \(activePeers)). This may take a few seconds. "
                        
                        hnsCertErrors[origin]  = NSError.init(domain: "HNSErrorNotReady", code: Int(result), userInfo: [
                            NSLocalizedDescriptionKey: errMessage
                        ])
                    } else if result == MobileHNSBogus {
                        let errMessage = verifyResult?.message() ?? ""
                        hnsCertErrors[origin]  = NSError.init(domain: "HNSErrorBogus", code: Int(result), userInfo: [
                            NSLocalizedDescriptionKey: "DANE Verify failed. \(errMessage)"
                        ])
                        
                    } else {
                        let errMessage = verifyResult?.message() ?? ""
                        
                        hnsCertErrors[origin]  = NSError.init(domain: "HNSVerifyError", code: Int(result), userInfo: [
                            NSLocalizedDescriptionKey: "Failed verifying site. \(errMessage)"
                        ])
                    }
                    
                    print("dane: host [\(challenge.protectionSpace.host)]: rejecting")
                    completionHandler(.cancelAuthenticationChallenge, nil)
                }
            }
